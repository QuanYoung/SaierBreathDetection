...
public class XArrayLocationMulti {
    ...
    public Settings GetPrepareSettings(ImpinjReader reader, XArray xArray) {
        ...
        settings.getXArray().getLocation().setEntryReportEnabled(true);
        settings.getXArray().getLocation().setUpdateReportEnabled(true);
        settings.getXArray().getLocation().setExitReportEnabled(true);
        // Enable Diagnostic reports here, soon to be deprecated
        settings.getXArray().getLocation().setDiagnosticReportEnabled(true);

        // Set xArray placement parameters
        // The mounting height of the xArray, in centimeters
        settings.getXArray().getPlacement().setHeightCm(xArray.Height);
        settings.getXArray().getPlacement().setFacilityXLocationCm(xArray.FacilityXcm);
        settings.getXArray().getPlacement().setFacilityYLocationCm(xArray.FacilityYcm);
        settings.getXArray().getPlacement().setOrientationDegrees(xArray.Orientation);
        // Compute Window and Gen2 Settings
        settings.getXArray().getLocation().setComputeWindowSeconds(COMPUTE_WINDOW_SEC);
        settings.setReaderMode(ReaderMode.AutoSetDenseReader);
        settings.setSession(xArray.Session);
        settings.getXArray().getLocation().setTagAgeIntervalSeconds(TAG_AGE_SEC);

        // Specify how often we want to receive location reports
        settings.getXArray().getLocation().setUpdateIntervalSeconds(UPDATE_INTERVAL_SEC);
