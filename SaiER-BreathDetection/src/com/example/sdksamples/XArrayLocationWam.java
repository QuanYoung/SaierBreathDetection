...
public class XArrayLocationWam {
    ...
    public void setupWamMode() {
        ...
        settings.getXArray().setMode(XArrayMode.WideAreaMonitor);
    ...
    public void setupLocationMode() {
        ...
        settings.getXArray().getPlacement().setHeightCm((short) XARRAY_HEIGHT);
        settings.getXArray().getPlacement().setFacilityXLocationCm(0);
        settings.getXArray().getPlacement().setFacilityYLocationCm(0);
        settings.getXArray().getPlacement().setOrientationDegrees((short) 0);

        // Motion Window and Tag age
        settings.getXArray().getLocation().setComputeWindowSeconds((short) COMPUTE_WINDOW);
        settings.setReaderMode(READER_MODE);
        settings.setSession(LOCATION_SESSION);
        settings.getXArray().getLocation().setTagAgeIntervalSeconds((short) TAG_AGE_INTERVAL);
        settings.getXArray().getLocation().setUpdateIntervalSeconds((short) UPDATE_INTERVAL);
